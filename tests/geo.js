...
it('Init for `geo.js`', async () => {
  try {
    ...
    result = await r.db(dbName).table(tableName).indexWait("location").run();

    let insert_docs = [];
    for(let i=0; i<numDocs; i++) {
      insert_docs.push({location: r.point(r.random(0, 1, {float: true}), r.random(0, 1, {float: true}))})
    }
    result = await r.db(dbName).table(tableName).insert(insert_docs).run();
...
it('`r.circle` should work - 2', async () => {
  try {
    ...
    let result: any = await r.circle(r.point(0, 0), 2, {numVertices: 40}).run();
    assert.equal(result.$reql_type$, "GEOMETRY")
    assert.equal(result.type, "Polygon")
...
it('`r.circle` should work - 3', async () => {
  try {
    let result: any = await r.circle(r.point(0, 0), 2, {numVertices: 40, fill: false}).run();
    assert.equal(result.$reql_type$, "GEOMETRY")
...
it('`r.circle` should work - 4', async () => {
  try {
    let result: any = await r.circle(r.point(0, 0), 1, {unit: "km"}).eq(r.circle(r.point(0, 0), 1000, {unit: "m"})).run();
    assert(result);
...
it('`distance` should work - 1', async () => {
  try {
    let result: any = await r.point(0, 0).distance(r.point(1,1)).run();
...
it('`distance` should work - 2', async () => {
  try {
    let result: any = await r.point(0, 0).distance(r.point(1,1), {unit: "km"}).run();
...
it('`distance` arity - 1', async () => {
  try {
    let result: any = await r.point(0, 0).distance().run();
    throw new Error("Was expecting an error");
...
it('`distance` arity - 2', async () => {
  try {
    let result: any = await r.point(0, 0).distance(1, 2, 3).run();
    throw new Error("Was expecting an error");
...
it('`fill` should work', async () => {
  try {
    let result: any = await r.circle(r.point(0, 0), 2, {numVertices: 40, fill: false}).fill().run();
    assert.equal(result.$reql_type$, "GEOMETRY")
    assert.equal(result.type, "Polygon")
    assert.equal(result.coordinates[0].length, 41)
...
it('`fill` arity error', async () => {
  try {
    let result: any = await r.circle(r.point(0, 0), 2, {numVertices: 40, fill: false}).fill(1).run();
    throw new Error("Was expecting an error");
...
it('`geojson` should work', async () => {
  try {
    let result: any = await r.geojson( {"coordinates":[0,0],"type":"Point"} ).run()
    assert.equal(result.$reql_type$, "GEOMETRY")
...
it('`toGeojson` should work', async () => {
  try {
    let result: any = await r.geojson( {"coordinates":[0,0],"type":"Point"}).toGeojson().run()
...
it('`toGeojson` arity error', async () => {
  try {
    let result: any = await r.point(0, 0).toGeojson(1, 2, 3).run()
    throw new Error("Was expecting an error");
...
it('`getIntersecting` should work', async () => {
  try {
    ...
    let result: any = await r.db(dbName).table(tableName).getIntersecting(r.polygon([0, 0], [0,1], [1,1], [1,0]), {index: "location"}).count().run()
...
it('`getIntersecting` arity', async () => {
  try {
    // All points are in [0,1]x[0,1]
    let result: any = await r.db(dbName).table(tableName).getIntersecting(r.polygon([0, 0], [0,1], [1,1], [1,0])).count().run()
    throw new Error("Was expecting an error");
...
it('`getNearest` should work', async () => {
  try {
    // All points are in [0,1]x[0,1]
    let result: any = await r.db(dbName).table(tableName).getNearest(r.point(0, 0), {index: "location", maxResults: 5}).run()
...
it('`getNearest` arity', async () => {
  try {
    let result: any = await r.db(dbName).table(tableName).getNearest(r.point(0, 0)).count().run()
    throw new Error("Was expecting an error");
...
it('`includes` should work', async () => {
  try {
    ...
    let result: any = await r.circle(point1, 2000).includes(point2).run();
    assert(result)
...
it('`includes` arity', async () => {
  try {
    let result: any = await r.circle([0,0], 2000).includes().run();
    throw new Error("Was expecting an error");
...
it('`polygonSub` should work', async () => {
  try {
    let result: any = await r.polygon([0, 0], [0, 1], [1, 1], [1, 0]).polygonSub(r.polygon([0.4, 0.4], [0.4, 0.5], [0.5, 0.5])).run();
    assert.equal(result.$reql_type$, "GEOMETRY");
    assert.equal(result.type, "Polygon");
...
it('`polygonSub` arity', async () => {
  try {
    let result: any = await r.polygon([0, 0], [0, 1], [1, 1]).polygonSub().run();
    throw new Error("Was expecting an error");
