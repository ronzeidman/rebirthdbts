...
//  `immidiateReturn` will always return a Cursor, and will do it immidiately
//    In the official and rethinkdbdash you need to wait for the first results to arrive after sending the query
//    This makes it impossible to cancel (unless you close the underlying connection)
// NOT DOING:
//  Support node < 8
//  Support callbacks.
...
//    use `await r.connect()` instead -
//      it can both initialize the pool (+ restart it if needed) or return a connection if you want to use both APIs
//      you can even use the two flavors together
