...
function parseParam(param) {
    ...
    if (Array.isArray(param)) {
        return [ql2_1.Term.TermType.MAKE_ARRAY, param.map(p => parseParam(p))];
    }
    ...
    if (util_1.isFunction(param)) {
        ...
        const term = [
            ql2_1.Term.TermType.FUNC,
            [
                [
                    ql2_1.Term.TermType.MAKE_ARRAY,
                    Array(param.length)
                        .fill(0)
                        .map((_, i) => i + nextVarId)
                ],
                parseParam(param(...Array(param.length)
                    .fill(0)
                    .map((_, i) => query_1.toQuery([ql2_1.Term.TermType.VAR, [i + 1]]))))
            ]
        ];
