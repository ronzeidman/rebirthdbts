    ...
    return (...args) => {
        ...
        if (query_1.isQuery(args[0]) && args[0].term[0] === enums_1.TermType.ARGS) {
            ...
            if (maxArgs !== -1 && argsLength > maxArgs) {
                throw new error_1.RebirthDBError(`\`${!currentTerm ? `r.${termName}` : termName}\` takes at most ${maxArgs} argument${maxArgs === 1 ? '' : 's'}, ${argsLength} provided${!currentTerm ? '.' : ' after:'}`, { term: currentTerm });
            }
            ...
            params.push(...args.map(x => param_parser_1.parseParam(x)));
        }
        const term = [termType];
        if (params.length > 0) {
            term[1] = params;
