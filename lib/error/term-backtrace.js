...
function backtraceTerm(term, head = true, backtrace) {
    ...
    switch (type) {
        ...
        case enums_1.TermType.FUNC: {
            ...
            if (globals_1.globals.backtraceType === 'lambda') {
                return getMarked(combineMarks `(${params.reduce(joinMultiArray, [
                    '',
                    ''
                ])}) => ${backtraceTerm(args[1], false, nextBacktrace(1, backtrace))}`, backtrace);
            }
            else {
                return getMarked(combineMarks `function(${params.reduce(joinMultiArray, [
                    '',
                    ''
                ])}) { return ${backtraceTerm(args[1], false, nextBacktrace(1, backtrace))} }`, backtrace);
            }
        }
...
function backtraceObject(obj, backtrace) {
    ...
    return combineMarks `{ ${Object.entries(obj)
        ...
        return getMarked(combineMarks `${snakeToCamel(key)}: ${backtraceTerm(val, false, next)}`);
    })
        .reduce(joinMultiArray, ['', ''])} }`;
}
