...
function backtraceTerm(term, head = true, backtrace) {
    ...
    switch (type) {
        case ql2_1.Term.TermType.MAKE_ARRAY: {
            if (!args) {
                return getMarked('');
            }
        ...
        case ql2_1.Term.TermType.VAR: {
            return getMarked(`var${args[0]}`, backtrace);
        }
        case ql2_1.Term.TermType.FUNCALL: {
            if (!args) {
                return getMarked('');
            }
        ...
        case ql2_1.Term.TermType.BRACKET: {
            if (!args) {
                return getMarked('');
            }
...
function backtraceQuery(query, backtrace) {
    ...
    switch (type) {
        case ql2_1.Query.QueryType.START:
            return backtraceTerm(term, true); // `${backtraceTerm(term)}.run(${backtraceObject(optarg)})`
        case ql2_1.Query.QueryType.SERVER_INFO:
            return ['conn.server()'];
        case ql2_1.Query.QueryType.NOREPLY_WAIT:
