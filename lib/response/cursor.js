...
class Cursor extends stream_1.Readable {
    ...
    _read() {
        ...
        this._next().then(row => this.push(row)).catch((err) => {
            if (error_1.isRebirthDBError(err) && err.type === types_1.RebirthDBErrorType.CURSOR_END) {
                this.push(null);
            }
            else {
                this.emit('error', err);
    ...
    async next() {
        if (this.emitting) {
            throw new error_1.RebirthDBError('You cannot call `next` once you have bound listeners on the Feed.');
        }
        if (this.closed) {
    ...
    async toArray() {
        if (this.emitting) {
            throw new error_1.RebirthDBError('You cannot call `toArray` once you have bound listeners on the Feed.');
        }
        const all = [];
        if (!this.results) {
            await this.resolve();
    ...
    async each(callback, onFinishedCallback) {
        if (this.emitting) {
            throw new error_1.RebirthDBError('You cannot call `each` once you have bound listeners on the Feed.');
        }
        if (this.closed) {
    ...
    async eachAsync(rowHandler, final) {
        if (this.emitting) {
            throw new error_1.RebirthDBError('You cannot call `eachAsync` once you have bound listeners on the Feed.');
        }
        if (this.closed) {
    ...
    async _next() {
        ...
        const results = this.results && this.type === 'Atom' && Array.isArray(this.results[0])
            ? this.results[0]
            : this.results;
        if (!this.results || util_1.isUndefined(this.results[this.position])) {
            this.close();
