...
class ServerConnectionPool extends events_1.EventEmitter {
    ...
    setOptions({ buffer = this.buffer, max = this.max, silent = this.silent, log = this.log, timeoutError = this.timeoutError, timeoutGb = this.timeoutGb, maxExponent = this.maxExponent }) {
        this.silent = silent;
        this.log = log;
        this.timeoutError = timeoutError;
        this.timeoutGb = timeoutGb;
    ...
    async queue(term, globalArgs = {}) {
        ...
        if (!openConnections) {
            throw this.reportError(new error_1.RebirthDBError('No connections available'), true);
        }
