...
class RebirthDBConnection extends events_1.EventEmitter {
    constructor(connectionOptions, { db = 'test', user = 'admin', password = '', timeout = 20, pingInterval = -1, silent = false, log = (message) => undefined } = {}) {
        ...
        this.socket = new socket_1.RebirthDBSocket({
            ...
            user,
            password: password
                ? Buffer.concat([Buffer.from(password), handshake_utils_1.NULL_BUFFER])
    ...
    async reconnect(options) {
        ...
        try {
            let timer;
            await Promise.race([
                new Promise(resolve => (timer = setTimeout(resolve, this.timeout * 1000))),
                this.socket.connect()
            ]);
            if (timer) {
                clearTimeout(timer);
            ...
            const error = new error_1.RebirthDBError('Unable to establish connection, see cause for more info.', {
                cause,
                type: types_1.RebirthDBErrorType.CONNECTION
            });
        ...
        if (this.socket.status !== 'open') {
            ...
            throw new error_1.RebirthDBError(`Failed to connect to ${this.connectionOptions.host}:${this.connectionOptions.port} in less than ${this.timeout}s.`, { type: types_1.RebirthDBErrorType.TIMEOUT });
        }
        this.startPinging();
        return this;
    }
        ...
        if (this.pingInterval > 0) {
            this.pingTimer = setTimeout(async () => {
                ...
                if (result.t !== enums_1.ResponseType.RUNTIME_ERROR ||
                    ...
                    result.r[0] !== 'ping') {
                    this.reportError(new error_1.RebirthDBError('Ping error', { responseType: result.t }));
                }
                if (this.pingTimer) {
                    this.startPinging();
                }
