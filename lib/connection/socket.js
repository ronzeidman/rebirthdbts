...
class RebirthDBSocket extends events_1.EventEmitter {
    constructor({ port = 28015, host = 'localhost', user = 'admin', password = handshake_utils_1.NULL_BUFFER } = {}) {
        super();
        this.runningQueries = [];
        this.isOpen = false;
        this.nextToken = 0;
    ...
    async connect(options = {}, { host = this.host, port = this.port } = {}) {
        this.host = host;
        this.port = port;
        if (this.socket) {
        ...
        this.socket = socket;
        await new Promise(resolve => socket.connect(Object.assign({ port, host }, options), resolve));
    ...
    stopQuery(token) {
        this.sendQuery([ql2_1.Query.QueryType.STOP], token);
        this.setData(token);
    ...
    handleData() {
        while (this.buffer.length >= 12) {
            ...
            if (response.t !== ql2_1.Response.ResponseType.SUCCESS_PARTIAL) {
                this.finishQuery(token);
            }
